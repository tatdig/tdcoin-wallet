...
package de.schildbach.tdcwallet.integration.android;
 ...
 */
public final class TdcoinIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/tdcoin-paymentrequest"; // BIP 71

    /**
     * Request any amount of Tdcoins (probably a donation) from user, without feedback from the app.
     * 
     * @param context
     *            Android context
     * @param address
    ...
    public static void request(final Context context, final String address) {
        final Intent intent = makeTdcoinUriIntent(address, null);

        start(context, intent);
    }

    /**
     * Request specific amount of Tdcoins from user, without feedback from the app.
     * 
     * @param context
     *            Android context
     * @param address
    ...
    /**
     * Request any amount of Tdcoins (probably a donation) from user, with feedback from the app. Result
     ...
     * 
     * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Tdcoin
    ...
    /**
     * Request specific amount of Tdcoins from user, with feedback from the app. Result intent can be
    ...
    /**
     * Put BIP70 payment message into result intent. Meant for usage by Tdcoin wallet applications.
     * 
     * @param result
     *            result intent
     * @param payment
    ...
    /**
     * Get BIP70 payment message from result intent. Meant for usage by applications initiating a Tdcoin
     * payment.
     * 
     * You can use the transactions contained in the payment to validate the payment. For this, you need your
     * own Tdcoin infrastructure though. There is no guarantee that the payment will ever confirm.
     * 
     * @param result
     *            result intent
    ...
    /**
     * Put transaction hash into result intent. Meant for usage by Tdcoin wallet applications.
     * 
     * @param result
     *            result intent
     * @param txHash
    ...
    /**
     * Get transaction hash from result intent. Meant for usage by applications initiating a Tdcoin payment.
     * 
     * You can use this hash to request the transaction from the Tdcoin network, in order to validate. For
     * this, you need your own Tdcoin infrastructure though. There is no guarantee that the transaction has
     * ever been broadcasted to the Tdcoin network.
     * 
     * @param result
     *            result intent
    ...
    private static Intent makeTdcoinUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("tdcoin:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No Tdcoin application found.\nPlease install Tdcoin Wallet.", Toast.LENGTH_LONG)
                .show();
