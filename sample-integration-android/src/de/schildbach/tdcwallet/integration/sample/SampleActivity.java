...
package de.schildbach.tdcwallet.integration.sample;

import org.tdcoin.protocols.payments.Protos;
...
public class SampleActivity extends Activity {
    ...
    private void handleDonate() {
        ...
        TdcoinIntegration.requestForResult(SampleActivity.this, REQUEST_CODE, addresses[0]);
    }
    ...
    private void handleRequest() {
        try {
            ...
            final NetworkParameters params = MainNetParams.get();//Address.getParametersFromAddress(addresses[0]);

            final Protos.Output.Builder output1 = Protos.Output.newBuilder();
            output1.setAmount(AMOUNT);
            output1.setScript(ByteString.copyFrom(ScriptBuilder.createOutputScript(Address.fromString(params, addresses[0])).getProgram()));

            final Protos.Output.Builder output2 = Protos.Output.newBuilder();
            output2.setAmount(AMOUNT);
            output2.setScript(ByteString.copyFrom(ScriptBuilder.createOutputScript(Address.fromString(params, addresses[1])).getProgram()));
            ...
            TdcoinIntegration.requestForResult(SampleActivity.this, REQUEST_CODE,
    ...
    protected void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        if (requestCode == REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                final String txHash = TdcoinIntegration.transactionHashFromResult(data);
                if (txHash != null) {
                    ...
                    if (TdcoinIntegration.paymentFromResult(data) != null)
