...
import org.tdcoinj.script.ScriptException;
...
public final class PaymentIntent implements Parcelable {
    ...
    public static PaymentIntent fromTdcoinUri(final TdcoinURI tdcoinUri) {
        final Address address = tdcoinUri.getAddress();
        final Output[] outputs = address != null ? buildSimplePayTo(tdcoinUri.getAmount(), address) : null;
        final String bluetoothMac = (String) tdcoinUri.getParameterByName(Bluetooth.MAC_URI_PARAM);
        final String paymentRequestHashStr = (String) tdcoinUri.getParameterByName("h");
        final byte[] paymentRequestHash = paymentRequestHashStr != null ? base64UrlDecode(paymentRequestHashStr) : null;

        return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, tdcoinUri.getLabel(),
                bluetoothMac != null ? "bt:" + bluetoothMac : null, null, tdcoinUri.getPaymentRequestUrl(),
                paymentRequestHash);
    }
