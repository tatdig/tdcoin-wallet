 ...
 */
public final class RequestTxInfoTask {
    private final Handler backgroundHandler;
    private final Handler callbackHandler;
    private final ResultCallback resultCallback;
    public String txHex = "";
    private static final Logger log = LoggerFactory.getLogger(RequestTxInfoTask.class);

    public interface ResultCallback {
        void onResult(String txHex);

        void onFail(int messageResId, Object... messageArgs);
    }

    public RequestTxInfoTask(final Handler backgroundHandler, final ResultCallback resultCallback) {
    ...
    public static class JsonRpcRequest {
        ...
        public JsonRpcRequest(final String method, final Array[] params) {
    ...
    public static class JsonRpcResponse {
        public int id;
        public TxJsonVerbose result;
        public Error error;

        public static class TxJsonVerbose {
    ...
    public void RequestTxInfo(final AssetManager assets, final String txid) {
...
/*                 final Address legacyAddress = LegacyAddress.fromKey(Constants.NETWORK_PARAMETERS, key);
               ...
               if (key.isCompressed()) {
                ...
                final List<Callable<String>> tasks = new ArrayList<>(servers.size());
                for (final ElectrumServer server : servers) {
                    tasks.add(new Callable<String>() {
                        @Override
                        public String call() throws IOException {
                            log.info("{} - trying to request tx info for {}", server.socketAddress,
                                    txid);
                            try (final Socket socket = connect(server)) {
                                ...
                                Object[] prm;
                                prm = new Object[]{ txid, true };
                                JsonRpcRequest request = new JsonRpcRequest(
                                        (int) (System.currentTimeMillis()/10000), "blockchain.transaction.get",
                                        prm );
                                ...
                                //for (final Script outputScript : outputScripts) {
                                     ...
                                     */
                                //}
                                log.info("{} - got {} hex, response:{}", server.socketAddress,response.result.hex,response.toString()/*, utxos.size(), utxos*/);
                                txHex = response.result.hex;
                                return txHex;
                ...
                /*final Multiset<String> countedUtxos = HashMultiset.create();
                int numSuccess = 0, numFail = 0, numTimeOuts = 0;
                for (Future<String> future : futures) {
                    if (!future.isCancelled()) {
                        try {
                            final String utxos = future.get();
                            if (utxos != null) {
                                countedUtxos.add(utxos);
                                numSuccess++;
                            } else {
                                numFail++;
                            }
...
*/
    ...
    protected void onResult(final String hex) {
        callbackHandler.post(new Runnable() {
            @Override
            public void run() {
                resultCallback.onResult(hex);
            }
        });
    }
